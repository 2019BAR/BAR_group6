...
```{r}
mvtWeek1$Date = as.character(mvtWeek1$Date) %>% as.Date("%m/%d/%y %H:%M") #類別轉字串，字串轉date」
ts = mvtWeek1$Date
...
table(format(ts,"%u"), format(ts,"%H")) %>% as.data.frame.matrix%>%d3heatmap(F,F,col=colorRamp(c('seagreen','lightyellow','red')))
#mac不能做
#
```
...
```{r}
table(mvtWeek1$LocationDescription=='GAS STATION',format(ts,'%u')) 
#如果寫這樣要怎麼排序？
#6
```
