...
```{r}
table(USDA$HighSodium) #高鹽分與非高鹽分食物群的分類計數表
```

```{r}
table(USDA$HighSodium, USDA$HighFat) #高鹽分、非高鹽分、高脂肪與非高脂肪含量食物的交互分類計數表
```

```{r}
tapply(USDA$Iron, USDA$HighProtein, mean, na.rm=TRUE) #高蛋白與非高蛋白的食物群分別的鐵含量平均數統計
```

```{r}
tapply(USDA$VitaminC, USDA$HighCarbs, max, na.rm=TRUE)
#高碳含量與非高碳含量食物群分別的最大值維他命C含量
```

```{r}
tapply(USDA$VitaminC, USDA$HighCarbs, summary, na.rm=TRUE)
#高碳含量與非高碳含量食物群分別的維他命C統計數據總結
```
<br>
