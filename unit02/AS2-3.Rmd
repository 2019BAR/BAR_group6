...
```{r}
sort(tapply(CPS$Hispanic, CPS$Race, sum)) %>% {.[.>250]}
# American Indian、Multiracial、Black、White都至少有250個西班牙裔受訪著
```
<br><br><hr>
...
```{r}
summary(CPS)
# MetroAreaCode、Married、Education、EmploymentStatus、Industry都有missing value
```
...
# The Married variable being missing is related to the Age value and the Citizenship value for the interviewee.
# 通常是直接用看的，取association的做法很少見
```
...
# Midwest有最高比率的受訪者不住在都會區
```

__§ 2.5__ Which state has a proportion of interviewees living in a non-metropolitan area closest to 30%?
```{r}
tapply(is.na(CPS$MetroAreaCode), CPS$State, mean) %>% {abs(.-0.3)}  %>% sort
# Wisconsin的受訪者居住於非都會區的比例接近30%
#{abs(.-0.3)} : 每個比率減0.3再取絕對值
```

Which state has the largest proportion of non-metropolitan interviewees, ignoring states where all interviewees were non-metropolitan?
```{r}
tapply(is.na(CPS$MetroAreaCode), CPS$State, mean) %>% {.[.!=1]} %>% sort
# Montana有最高比例的受訪者居住於非都會區(全部受訪者皆住在非都會區的地區除外)
# {.[.!=1]}：除去比率=1的 再排序
```
...
# New York-Northern New Jersey-Long Island有最多受訪者
```
...
# Vallejo-Fairfield、San Jose-Sunnyvale-Santa Clara、San Francisco-Oakland-Fremont、Honolulu 有至少20%的受訪者為亞洲人
```
...
```{r}
table(CPS$Country[CPS$MetroArea=="New York-Northern New Jersey-Long Island, NY-NJ-PA"] == "United States") %>% prop.table
# 有0.3087的比例非美國
```
...
# Detroit-Warren-Livonia有最多生於India的受訪者
```

In Brazil?
```{r}
tapply(CPS$Country=="Brazil", CPS$MetroArea, sum) %>% sort %>% tail
# Boston-Cambridge-Quincy有最多生於Brazil的受訪者
```

In Somalia?
```{r}
tapply(CPS$Country=="Somalia", CPS$MetroArea, sum) %>% sort %>% tail
# Phoenix-Mesa-Scottsdale和St.Cloud有最多生於Somalia的受訪者
```
