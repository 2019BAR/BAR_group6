...
IBM$Date = as.character(IBM$Date) %>% as.Date('%m/%d/%y') #類別轉字串，字串轉date
GE$Date = as.character(GE$Date) %>% as.Date('%m/%d/%y')
ProcterGamble$Date = as.character(ProcterGamble$Date) %>% as.Date('%m/%d/%y')
CocaCola$Date = as.character(CocaCola$Date) %>% as.Date("%m/%d/%y")
Boeing$Date = as.character(Boeing$Date) %>% as.Date("%m/%d/%y")
...
max(GE$Date) %>% format("%Y")
max(CocaCola$Date) %>% format("%Y")
max(ProcterGamble$Date) %>% format("%Y")
max(Boeing$Date) %>% format("%Y")
#2009
...
```{r}
plot(CocaCola$Date,CocaCola$StockPrice,type = 'l',col='orange',ylim = c(0,500))
#ylim可以條兼具！
lines(GE$Date,GE$StockPrice,type = 'l',col='green')
lines(Boeing$Date,Boeing$StockPrice,type = 'l',col='pink') 
lines(IBM$Date,IBM$StockPrice,type = 'l',col='blue')
lines(ProcterGamble$Date,ProcterGamble$StockPrice,type = 'l',col='gray')
abline(v=as.Date(c("2000-03-01","1983-01-01","1984-01-01")),col="cyan",lty=2)
#lty是虛線的意思，數字控制虛線長度
...
```{r}
plot(Boeing$Date,Boeing$StockPrice,col="orange",type = "l",ylim =c(0,500))
lines(GE$Date,GE$StockPrice,type = 'l',col='green')
lines(Boeing$Date,Boeing$StockPrice,type = 'l',col='pink')
lines(IBM$Date,IBM$StockPrice,type = 'l',col='blue')
lines(ProcterGamble$Date,ProcterGamble$StockPrice,type = 'l',col='gray')
abline(v=as.Date(c("2000-03-01")),col="cyan",lty=2)
#GE
#
```
...
lines(IBM$Date,IBM$StockPrice,type = 'l',col='blue')
lines(ProcterGamble$Date,ProcterGamble$StockPrice,type = 'l',col='gray')
abline(v=as.Date(c("1995-01-01","2005-01-01")),col="cyan",lty=2)
...
lines(IBM$Date,IBM$StockPrice,type = 'l',col='blue')
lines(ProcterGamble$Date,ProcterGamble$StockPrice,type = 'l',col='gray')
abline(v=as.Date(c("1997-9-01","1997-11-01")),col="cyan",lty=2)
#GE
...
```{r}
plot(Boeing$Date,Boeing$StockPrice,col="orange",type = "l",ylim=c(0,120),
     xlim = as.Date(c("2004-01-01","2005-12-31")))
lines(GE$Date,GE$StockPrice,type = 'l',col='green')
lines(Boeing$Date,Boeing$StockPrice,type = 'l',col='pink') 
lines(IBM$Date,IBM$StockPrice,type = 'l',col='blue')
lines(ProcterGamble$Date,ProcterGamble$StockPrice,type = 'l',col='gray')
abline(v=as.Date(c("2004-01-01","2005-12-31")),col="cyan",lty=2)
#Boeing
```
...
```{r}
tapply(IBM$StockPrice,format(IBM$Date,"%m") , mean) %>% sort(decreasing = TRUE) %>% head(1)
...
max(tapply(CocaCola$StockPrice, format(CocaCola$Date,"%m"), mean))
...
```{r}
sapply(L, function(df){tapply(df$StockPrice,format(df$Date,"%m"),mean)})

#lapply(L, function(df){tapply(df$StockPrice,format(df$Date,"%m"),mean)})
#兩者差在表現形式，sapply會變成矩陣的樣子
#12月較低
```
<br>
<p class="wwl">
